...
{
    // (Removed: a temporary unconditional per-packet RX log lived here
    // while diagnosing SDR Console's frequency behavior. It confirmed SDR
    // Console sends zero packets of any kind between the initial connect
    // burst and a disconnect/reconnect - not a minibitx parsing bug, a
    // real client limitation. See radio.c/README notes on Hamlib/rigctld
    // as the planned fix: SDR Console almost certainly expects live
    // frequency control over a separate CAT connection, with the IQ link
    // treated as a passive audio/spectrum source only.)
    ...
    case 0x01:  // EP2 host commands
        {
            ...
            for (int frame = 0; frame < frames_available; frame++) {
                ...
                uint32_t val = ((uint32_t)fp[4] << 24) | ((uint32_t)fp[5] << 16) |
                               ((uint32_t)fp[6] <<  8) |  (uint32_t)fp[7];

                // Confirmed against real captured traffic during earlier
                // diagnosis: addr 2 carries real RX frequency in every
                // client tested so far - it tracked the SDR app's actual
                // dial frequency exactly. addr 0 does NOT - it's a packed
                // general control/status word (flag bits, not a swept
                // number), and treating it as frequency caused spurious
                // retunes to garbage values (e.g. 4 Hz, 16.7 MHz)
                // interleaved with the real frequency from addr 2 - that
                // was the "frequencies don't look right" bug. addr 1 also
                // looked plausible as a frequency in captured data but
                // isn't handled yet - add it here if a future client turns
                // out to need it, once confirmed the same way.
                if (addr == 0x02) {
                    int f = (int)val;
